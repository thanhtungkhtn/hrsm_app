...
export default {
  ...
  actions: {
    ...
    async savePosition({dispatch, commit, state}, position) {
      ...
      return HTTP().patch(`/position/${position.id}`, {
        name: state.currentPositionName ? state.currentPositionName : position.name,
        basic_salary: state.currentPositionBasicSalary ? state.currentPositionBasicSalary : position.basic_salary,
        position_coefficient: state.currentPositionHeSoChucVu ? state.currentPositionHeSoChucVu : position.position_coefficient,
        responsibility_coefficient: state.currentPositionHeSoTrachNhiem ? state.currentPositionHeSoTrachNhiem : position.responsibility_coefficient,
      })
        .then( async () => {
    ...
    async deletePosition({commit}, position) {
      return HTTP().delete(`/position/${position.id}`)
        .then((res) => {
          res.data ? commit('removePosition', position) : alert("Cannot delete because of a logical error!!")
        });
    },
  },
  getters: {
  ...
  mutations: {
    ...
    removePosition(state, position) {
      try {
        state.positions.results.splice(state.positions.results.findIndex(x => x === position), 1);
      }
      catch(err) {
        console.log(err)
      }
