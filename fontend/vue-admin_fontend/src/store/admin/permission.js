...
export default {
  ...
  actions: {
    ...
    async savePermission({dispatch, commit, state}, permission) {
      ...
      return HTTP().patch(`/permission/${permission.id}`, {
        name_per: state.currentPermissionName ? state.currentPermissionName : permission.name, //isMember ? "$2.00" : "$10.00"
    ...
    async deletePermission({commit}, permission) {
      return HTTP().delete(`/permission/${permission.id}`)
        .then((res) => {
          res.data ? commit('removePermission', permission) : alert("Cannot delete because of a logical error!!")
        });
    },
    ...
    async savePermissionDetail({dispatch, commit, state}, permission_detail) {
      ...
      return HTTP().patch(`/perdet/${permission_detail.id}`, {
        action_name: state.currentPermissionDetailActionName ? state.currentPermissionDetailActionName : permission_detail.action_name,
        action_code: state.currentPermissionDetailActionCode ? state.currentPermissionDetailActionCode : permission_detail.action_code,
        check_action: state.currentPermissionDetailCheckAction ? state.currentPermissionDetailCheckAction : permission_detail.check_action,
        // name_per: state.currentPermissionName ? state.currentPermissionName : permission_detail.name, //isMember ? "$2.00" : "$10.00"
        // relationship: state.currentRelationship ? state.currentRelationship : relation.relationship,
      })
        .then(async () => {
          await dispatch('fetchPermissions')
    ...
    async deletePermissionDetail({ commit }, permission_detail) {
      return HTTP().delete(`/perdet/${permission_detail.id}`)
        .then((res) => {
          res.data ? commit('removePermissionDetail', permission_detail) : alert("Cannot delete because of a logical error!!")
        });
    },
  },
  getters: {
  ...
  mutations: {
    setNewPermissionName(state, name) {
      ...
      // console.log(state.newPermissionName)
    },
    ...
    removePermission(state, permission) {
      try {
        state.permissions.results.splice(state.permissions.results.findIndex(x => x === permission), 1);
      }
      catch(err) {
        console.log(err)
      }
    ...
    removePermissionDetail(state, permission_detail) {
      try {
        // console.log(permission_detail.permision_id)

        state.permissions.results.map(x => {
          // console.log(x.permisionDetail)
          x.permisionDetail.splice(x.permisionDetail.findIndex(x => x === permission_detail), 1)
        })
      }
      catch(err) {
        console.log(err)
