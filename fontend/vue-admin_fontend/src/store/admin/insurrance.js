...
export default {
  ...
  actions: {
    ...
    async saveInsurrance({ dispatch, commit, state }, insurrance) {
      ...
      return HTTP()
        .patch(`/insurrance/${insurrance.id}`, {
          type: state.currentInsurranceType
            ? state.currentInsurranceType
            : insurrance.type,
          card_number: state.currentInsurranceCardNumber
            ? state.currentInsurranceCardNumber
            : insurrance.card_number,
          insurance_money: state.currentInsurranceInsuredValue
            ? state.currentInsurranceInsuredValue
            : insurrance.insurance_money,
          day_of_issue: state.currentInsurranceDayOfIssue
            ? state.currentInsurranceDayOfIssue.substring(0, 10)
            : insurrance.day_of_issue.substring(0, 10),
          expiration_date: state.currentInsurranceExpirationDate
            ? state.currentInsurranceExpirationDate.substring(0, 10)
            : insurrance.expiration_date.substring(0, 10),
          place_of_issue: state.currentInsurrancePlaceOfIssue
            ? state.currentInsurrancePlaceOfIssue
            : insurrance.place_of_issue
        })
        .then(async () => {
    ...
    async deleteInsurrance({ commit }, insurrance) {
      return HTTP()
        .delete(`/insurrance/${insurrance.id}`)
        .then(res => {
          res.data
            ? commit("removeInsurrance", insurrance)
  ...
  mutations: {
    ...
    removeInsurrance(state, position) {
      try {
        state.insurrances.results.splice(
          state.insurrances.results.findIndex(x => x === position),
          1
        );
      } catch (err) {
        console.log(err);
