...
export default {
  ...
  actions: {
    ...
    async saveOffice({ dispatch, commit, state}, office) {
      // console.log(office)
      return HTTP().patch(`/office/${office.id}`, {
    ...
    async deleteOffice({commit}, office) {
      return HTTP().delete(`/office/${office.id}`)
        .then((res) => {
          // console.log(res.data)
          res.data ? commit('removeOffice', office) : alert("Cannot delete because of a logical error!!")
        });
    },
  },
  getters: {
  ...
  mutations: {
    ...
    removeOffice(state, office) {
      try {
        state.offices.results.splice(state.offices.results.findIndex(x => x === office), 1);
      }
      catch(err) {
        console.log(err)
      }
